...
Use the Library quick reference table to find the essential overrides for the target library. For complete code examples and helper functions, read ONE rule file from `references/references/rules/` matching the library.
...
The universal rules above are sufficient for most charts. For complete code examples and library-specific helpers, read the appropriate rule file from the `references/references/rules/` directory in this skill's folder. Only read ONE rule file per task.

| Library | Rule file to read | Essential config (apply even without reading the file) |
|---------|-------------------|--------------------------------------------------------|
| Recharts | `references/references/rules/recharts.md` | `<CartesianGrid stroke="none" />`, remove `<Legend />`, `<YAxis axisLine={false} tickLine={false} />`, `<Line dot={false} strokeWidth={1.5} />` |
| ECharts | `references/references/rules/echarts.md` | `splitLine: { show: false }`, `legend: { show: false }`, `grid: { show: false }`, use `endLabel` on series |
| Chart.js | `references/references/rules/chartjs.md` | `grid: { display: false }`, `border: { display: false }`, `plugins.legend.display: false`, use `chartjs-plugin-datalabels` |
| matplotlib | `references/references/rules/matplotlib.md` | `spines['top'].set_visible(False)`, `spines['right'].set_visible(False)`, `spines['bottom'].set_bounds(min, max)`, `font.family: serif` |
| Plotly | `references/references/rules/plotly.md` | `showgrid=False`, `showlegend=False`, `plot_bgcolor='#fffff8'`, `zeroline=False` |
| D3/SVG/HTML | `references/references/rules/svg-html.md` | `.domain { display: none }`, no `<rect>` backgrounds, `stroke-opacity: 0.1` for any gridlines |
...
For small multiples, sparklines, and slopegraph implementation patterns, see `references/references/rules/small-multiples-sparklines.md`.
...
Font stacks in rule 10. For full palettes (sequential, diverging), font loading, and old-style figures, see `references/references/rules/typography-and-color.md`.
...
For the full table with per-library detection patterns and one-liner fixes, see `references/references/rules/anti-patterns.md`.
...
**Library rules** (read ONE per task): `references/references/rules/recharts.md`, `references/references/rules/echarts.md`, `references/references/rules/chartjs.md`, `references/references/rules/matplotlib.md`, `references/references/rules/plotly.md`, `references/references/rules/svg-html.md` — complete code examples, helpers, and theme registrations.

**Cross-cutting** (read when specifically needed):
- `references/references/rules/interactive-and-accessible.md` — progressive disclosure, WCAG, responsive, animation, dark mode
- `references/references/rules/typography-and-color.md` — font loading, full palette tables, old-style figures
- `references/references/rules/anti-patterns.md` — per-library detection heuristics and fixes
- `references/references/rules/small-multiples-sparklines.md` — layout patterns for small multiples, sparklines, slopegraphs

**Working examples** in `references/references/examples/` — one per library, plus an inline SVG sparkline.
