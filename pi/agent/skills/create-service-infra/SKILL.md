...
description: >
  ...
  repository. Creates all required PRs in a single pass for one
...
Every branch uses the format `feature/{linear_ticket}-{suffix}`.
...
2. Create the exact branch specified by the applicable PR section.
...
8. Push the branch.
9. Create the PR through the GitHub API.
10. Include the required description fields from section 7.
11. Return to the default branch before creating the next branch.

Do not wait for Atlantis, CI, merging, or user input between independent PRs.
Do not run OpenTofu or Terraform plan/apply. Dry-run performs none of these
mutating steps.
...
- Branch: `feature/{linear_ticket}-ecr`.
...
- Do not add an Atlantis project.
- Dependencies: `none — can merge immediately`.
- Branch: `feature/{linear_ticket}-secretsmanager`.
...
- Do not add an Atlantis project.
- Dependencies: `none — can merge immediately`.
- Branch: `feature/{linear_ticket}-s3`.
...
- Do not add an Atlantis project.
- Dependencies: `none — can merge immediately`.
- Branch: `feature/{linear_ticket}-internal-routes`.
...
- Dependencies: ECR must be merged and applied first when an ECR PR exists; otherwise `none`.
- Branch: `feature/{linear_ticket}-ecs-service`.
...
- Dependencies: ECS must be merged and applied first.
- Branch: `feature/{linear_ticket}-postgresql`.
...
3. `Creates: {specific resources and files}`
4. `Dependencies: {required PRs, or "none — can merge immediately"}`
5. `Merge order: {explicit order or parallel instruction}`
6. `Atlantis: {existing stack or new project entries}`
...
Summarize merge order:

1. ECR, Secrets Manager, S3, and Internal Routes can merge in parallel.
2. ECS waits for ECR to be merged and applied when ECR was created.
3. PostgreSQL waits for ECS to be merged and applied when a new database was created.
4. PostgreSQL Atlantis applies the cluster before the database.
...
| PR | Branch | Commit | Files | Dependencies |
|---|---|---|---|---|
| ECR (when the mapping is absent) | `feature/DAT-1625-ecr` | `feat(ecr): add demo-api repository` | `global/ecr/locals.tf` | `none — can merge immediately` |
| Secrets Manager | `feature/DAT-1625-secretsmanager` | `feat(secretsmanager): add demo-api secrets` | `staging/secretsmanager/locals.tf` | `none — can merge immediately` |
| Internal Routes | `feature/DAT-1625-internal-routes` | `feat(routes): add demo-api route` | `staging/internal-service-routes/locals.tf` | `none — can merge immediately` |
| ECS Service | `feature/DAT-1625-ecs-service` | `feat(ecs): add demo-api service` | exactly the eight API files under `staging/ecs/shared/services/demo-api`, plus one inherited Atlantis entry | ECR merged and applied first when the ECR PR exists; otherwise `none` |
| PostgreSQL | `feature/DAT-1625-postgresql` | `feat(postgresql): add demo database` | five cluster files, six database files, plus two inherited Atlantis entries | ECS merged and applied first |

No S3, application-repository, inventory, module, or state PR is planned.
...
- [ ] Exact five-row PR matrix, including conditional ECR and exact dependencies.
...
- Never auto-rebase or force-push.
